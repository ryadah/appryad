...
class MainActivity : Activity() {
    ...
    private fun saveOfflineStateInternal(json: String) {
        val dir=offlineDir(); if(!dir.exists()) dir.mkdirs()
        val tmp=File(dir,"state.json.tmp")
        tmp.writeText(json,Charsets.UTF_8)
        if(!tmp.renameTo(offlineStateFile())) { tmp.copyTo(offlineStateFile(),overwrite=true); tmp.delete() }
    }

    private fun downloadAttachmentToApp(urlString: String, attachmentId: String, fileName: String, mime: String) {
        Thread {
            try {
                if (attachmentId.isBlank() || urlString.isBlank()) throw IllegalArgumentException("invalid_attachment")
                val dir=File(attachmentDir(), attachmentId); if(!dir.exists()) dir.mkdirs()
                val target=File(dir,safeFileName(fileName)); val part=File(dir,target.name+".part")
                if(target.exists() && target.length()>0L){
                    runOnUiThread { webView.evaluateJavascript("try{onAndroidAttachmentStored(${org.json.JSONObject.quote(attachmentId)},true)}catch(e){}",null) }
                    return@Thread
                }
                val connection=(URL(urlString).openConnection() as HttpURLConnection).apply{
                    requestMethod="GET"; connectTimeout=15000; readTimeout=120000; instanceFollowRedirects=true
                    if(part.exists() && part.length()>0) setRequestProperty("Range","bytes=${part.length()}-")
                }
                connection.connect()
                val append=connection.responseCode==206 && part.exists()
                if(connection.responseCode !in 200..299) throw IllegalStateException("HTTP ${connection.responseCode}")
                connection.inputStream.use { input ->
                    RandomAccessFile(part,"rw").use { raf ->
                        if(append) raf.seek(part.length()) else { raf.setLength(0); raf.seek(0) }
                        val buf=ByteArray(1024*64); while(true){ val n=input.read(buf); if(n<0) break; raf.write(buf,0,n) }
                    }
                }
                connection.disconnect()
                if(!part.exists() || part.length()==0L) throw IllegalStateException("empty_attachment")
                if(!part.renameTo(target)){part.copyTo(target,overwrite=true);part.delete()}
                runOnUiThread { webView.evaluateJavascript("try{onAndroidAttachmentStored(${org.json.JSONObject.quote(attachmentId)},true)}catch(e){}",null) }
            } catch(e:Exception){
                runOnUiThread { webView.evaluateJavascript("try{onAndroidAttachmentStored(${org.json.JSONObject.quote(attachmentId)},false)}catch(e){}",null) }
            }
        }.start()
    }

    private fun openStoredAttachment(attachmentId: String, fileName: String, mime: String) {
        try {
            val dir=File(attachmentDir(),attachmentId); val file=dir.listFiles()?.firstOrNull { !it.name.endsWith(".part") }
                ?: throw IllegalStateException("not_downloaded")
            val uri=FileProvider.getUriForFile(this,"${packageName}.fileprovider",file)
            val intent=Intent(Intent.ACTION_VIEW).apply{setDataAndType(uri,mime.ifBlank{"application/octet-stream"});addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)}
    ...
    inner class AndroidBridge {
        ...
        @JavascriptInterface
        fun getOfflineState(): String = try { if(offlineStateFile().exists()) offlineStateFile().readText(Charsets.UTF_8) else "" } catch(_:Exception){ "" }

        @JavascriptInterface
        fun saveOfflineState(stateJson: String) {
            if(stateJson.isBlank()) return
            Thread { try { saveOfflineStateInternal(stateJson) } catch(_:Exception){} }.start()
        }
        ...
        @JavascriptInterface
        fun downloadAndStoreAttachment(url: String, attachmentId: String, fileName: String, mime: String?) {
            downloadAttachmentToApp(url,attachmentId,fileName,mime ?: "application/octet-stream")
        }

        @JavascriptInterface
        fun openStoredAttachment(attachmentId: String, fileName: String, mime: String?) {
            runOnUiThread { this@MainActivity.openStoredAttachment(attachmentId,fileName,mime ?: "application/octet-stream") }
        }
